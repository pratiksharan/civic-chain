...
// Matches CivicChain_v5 sidebar: slides to width 0 when closed, "Navigation"
// section label, active dot indicator, settings separated by a divider, and
// hover effect via nav-btn CSS class. No lock emoji — uses "—" for locked items.
...
export function Sidebar({ screen, unlocked, city, sidebarOpen, onNavigate }: SidebarProps) {
  return (
    <div
      style={{
      ...
      <nav style={{ padding: '14px 10px', flex: 1, overflowY: 'auto' }}>
        ...
        {MAIN_NAV.map((item) => {
          ...
          return (
            <button
              key={item.id}
              onClick={() => !isLocked && onNavigate(item.id as Screen)}
              className="nav-btn"
              style={{
                display: 'flex',
                alignItems: 'center',
