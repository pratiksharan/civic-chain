...
import type { Screen, AuthScreen as AuthScreenType, IssueAnalysis, LoadingState, SettingsValues } from '@/types';
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: T.bg, fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", fontSize: 14, color: T.txt }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '32px 36px' }}>
          {screen === 'dashboard' && (
            <Dashboard analysis={analysis} dept={dept} unlocked={unlocked} city={city} onNavigate={navigate} />
          )}

          {screen === 'submit' && (
            <SubmitIssue
              issueText={issueText} onIssueChange={setIssueText}
              onAnalyze={handleAnalyze} loading={!!loading.analyze} city={city}
            />
          )}
          ...
          {screen === 'routing' && analysis && dept && (
            <RoutingResult
              ...
              loading={loading} onGenerateComplaint={handleGenerateComplaint}
