...
export function SubmitIssue({ issueText, onIssueChange, onAnalyze, loading, city, isMobile }: SubmitIssueProps) {
  return (
    <div style={{ maxWidth: 820 }} className="animate-fade-in">
      <h1 style={{ fontSize: isMobile ? 22 : 24, fontWeight: 700, color: T.txt, margin: 0, marginBottom: 5, letterSpacing: '-0.02em' }}>
        Submit Issue
      </h1>
          ...
          <Card>
            ...
            <textarea
              ...
              style={{
                ...
                background: T.bg, border: `1px solid ${T.bord}`, borderRadius: 7,
                color: T.txt, fontFamily: 'inherit', fontSize: 14, padding: 14,
            ...
            <div style={{ marginTop: 16 }}>
              <PrimaryBtn onClick={onAnalyze} loading={loading} style={{ width: '100%', justifyContent: 'center', padding: '12px 0', fontSize: 14 }}>
                Analyse Issue
              </PrimaryBtn>
            </div>
            {loading && <div style={{ marginTop: 14 }}><Spinner /></div>}
          </Card>
        </div>

        {/* How it works — below on mobile */}
        <div>
          <Card>
            <SL>How it works</SL>
            {HOW_IT_WORKS.map(([n, t, d]) => (
              <div key={n} style={{ display: 'flex', gap: 12, marginBottom: 14 }}>
                <div style={{ width: 22, height: 22, borderRadius: 5, background: T.tealLight, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, color: T.teal, fontWeight: 700, flexShrink: 0 }}>
                  {n}
                </div>
                <div>
